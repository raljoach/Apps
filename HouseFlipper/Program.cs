...
namespace HouseFlipper
{
    class Program
    {
        ...
        private static void ReadFiles(string[] files, List<MlsRow> mlsRows, Dictionary<string, MlsRow> soldHash, Dictionary<string, Dictionary<string, List<MlsRow>>> flippedHash)
        {
            foreach (var f in files)
            {
                ...
                if (rowNum == 0)
                {
                    Logger.Debug("Empty file: '{0}'", f);
                }
                else if (rowNum == 1)
                {
                    Logger.Debug("Header row found, but no data rows exist in file:'{0}'", f);
                }
            }
        }
